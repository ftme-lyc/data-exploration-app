...
with st.expander("ℹ️ - Streamlit application for performing data exploration on a CSV", expanded=True):
    ...
    if uploaded_file:
        try:
            st.success("File uploaded and dataset loaded successfully!")
            st.session_state.file_path = uploaded_file  # Save file in session state for potential future use
...
if st.session_state.file_path is not None:
    ...
    with tab_num:
        display_tab_num_content(df=st.session_state.dataset)
    with tab_text:
        display_tab_text_content(df=st.session_state.dataset)
    with tab_date:
        display_tab_date_content(df=st.session_state.dataset, key='date_tab')
else:
